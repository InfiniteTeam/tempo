...
export default new Modal('modal.voice-rename', async (client, interaction) => {
  ...
  const { isPrivate } = await voiceManager.getVoiceChannelState(
    interaction.channel
  )
  ...
  try {
    ...
    await interaction.reply({
      embeds: [
        new Embed(client, 'error').setTitle('채널 이름 설정에 실패했어요.')
      ],
      ephemeral: true
    })
    return
  ...
  await client.db.redis.set(
    VoiceManager.getRedisKey('rename', interaction.channel.id),
    '1',
    'EX',
    VoiceManager.getRedisExpire('rename')
  )

  await interaction.channel?.send({
    embeds: [
      new Embed(client, 'success').setTitle(
        `채널 이름이 \`${newName}\` 으로 변경되었어요.`
      )
    ]
  })
})
